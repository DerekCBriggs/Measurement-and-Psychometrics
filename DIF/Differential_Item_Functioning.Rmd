...
The test-level plots show the impact of DIF items on the test characteristic curves (TCCs). When DIF operates in opposite directions across items, the effects may cancel at the test level, resulting in minimal impact on total scores even though individual items show significant DIF.

### Individual-Level Impact

The individual-level impact plot compares each person's initial "naive" $\hat{\theta}$ (ignoring DIF) to their "purified" $\hat{\theta}$ (accounting for DIF with group-specific item parameters). Small differences indicate that DIF has minimal practical impact on individual scores.

---

## Are the Differences Practically Significant?

Even when items are flagged as statistically significant, the practical significance of DIF should be assessed by comparing the magnitude of score changes to the standard error of measurement (SEM). If the change in $\hat{\theta}$ attributable to DIF is much smaller than the SEM, the impact on individual decisions is negligible.
